...
// Decoding every stored vector costs a ~200,000-iteration loop on each cold
// isolate, and on low, sporadic traffic every request IS a cold isolate. Most
// of those vectors are never scored, so decode one at a time on demand and
// keep it. Same numbers as unpackVectors, a fraction of the work.
export function lazyVectors(b64, dims, count) {
  const bin = atob(b64);
  const seen = new Array(count);
  return {
    count,
    get(i) {
      ...
      const off = i * dims;
      // Assigning a 0..255 char code to an Int8Array wraps it to -128..127,
      // which is exactly the signed byte that was packed.
      for (let j = 0; j < dims; j++) v[j] = bin.charCodeAt(off + j);
      seen[i] = v;
      return v;
    },
  };
